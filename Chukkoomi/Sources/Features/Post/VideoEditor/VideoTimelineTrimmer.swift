...
struct VideoTimelineTrimmer: View {
    ...
    var body: some View {
        GeometryReader { geometry in
            ...
            ZStack(alignment: .leading) {
                ...
                .onAppear {
                    if duration > 0 && geometry.size.width > 0 {
                        loadThumbnails(for: geometry.size)
                    }
                }
                .onChange(of: duration) { _, newDuration in
                    if newDuration > 0 && geometry.size.width > 0 {
                        loadThumbnails(for: geometry.size)
                    }
                }
                ...
                .frame(height: geometry.size.height)
                .offset(x: endPosition - handleWidth)
                .gesture(
                    DragGesture()
                        .onChanged { value in
                            isDraggingEnd = true
    ...
    private func loadThumbnails(for size: CGSize) {
        ...
        PHImageManager.default().requestAVAsset(forVideo: videoAsset, options: options) { avAsset, _, _ in
            ...
            let thumbnailWidth = (size.width / CGFloat(self.thumbnailCount)) * scale
            ...
            let interval = self.duration / Double(self.thumbnailCount)
